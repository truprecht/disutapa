...
def test_parsing():
    ...
    tree = Derivation(r1, 1, (None, None, Derivation(r2, 2, (None,))))

    assert list(DopTreeParser._match(fragments[2], tree.children[2])) == []
    assert list(DopTreeParser._match(fragments[0], tree)) == []
    assert list(DopTreeParser._match(fragments[1], tree)) == [tree.children[2]]
    ...
    assert set(parser.chart.keys()) == {1,2}
    assert parser.chart[1] == 0
    assert parser.chart[2] == float("-inf")

    parser = DopTreeParser(Dop(trees, prior=1))
    parser.fill_chart(trees[0])

    assert set(parser.chart.keys()) == {1,2,3,4}
    assert parser.chart[1] == parser.chart[2] + parser.chart[3] + log(4)
    assert parser.chart[2] == log(4)
    assert parser.chart[3] == log(4) - log(3)
    assert parser.chart[4] == log(4)