...
def test_extract():
    ...
    t[1].type = HEAD
    t = HeadedTree.convert(t)
    r1 = headed_rule("ROOT", ["ARG"], headed_clause(Tree("S", [1, 0])), 0)
    r2 = headed_rule("ARG", [], headed_clause(0), 0)
    ...
    assert deriv.label == (3, headed_rule("ROOT", ["VP", "NP"], headed_clause(Tree("SBAR+S", [Tree("VP", [1, 0]), 2])), 0))
    assert deriv[0].label == (4, headed_rule("VP", ["ARG", "ARG"], headed_clause(Tree("VP", [1, 0, 2])), 0))
    assert deriv[(0,0)].label == (0, headed_rule("ARG", [], headed_clause(0), 0))
    assert deriv[(0,1)].label == (5, headed_rule("ARG", [], headed_clause(0), 0))
    assert deriv[1].label == (2, headed_rule("NP", ["ARG"], headed_clause(Tree("NP", [1, 0])), 0))
    assert deriv[(1,0)].label == (1, headed_rule("ARG", [], headed_clause(0), 0))
