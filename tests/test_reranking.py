...
from sdcp.reranking import classifier
from sdcp.reranking import svm
...
def test_ranker():
    ...
    # ranker.fit(20, classifier.max_margin_loss, weight_decay=1e-1, devset=list(zip(goldindices, silvertrees)))
    # for gold, goldidx, silvers in zip(goldtrees, goldindices, silvertrees):
    #     i, tree = ranker.select(silvers)
    #     assert i == goldidx, f"chose tree {tree} instead of {gold}"
...
#     ranker = svm.TreeRanker(min_feature_count=1)
#     for gold, silvers in zip(goldtrees, silvertrees):
#         ranker.add_tree(gold, silvers)
    
#     ranker.fit(devset=list(zip(goldindices, silvertrees)))
#     for gold, goldidx, silvers in zip(goldtrees, goldindices, silvertrees):
#         i, tree = ranker.select(silvers)
#         assert i == goldidx, f"chose tree {tree} instead of {gold}"